...
C ALPD       = Half-cone angle (degrees)
...
Cf2py intent(in)  ALPD,TH0D,R1,R2,IT,JP,M1,M2,Yinit,LOADP,REV,NT
Cf2py intent(in)  XH,YH,ZH,V1,V2,PHI1,PHI2,H0,H1,PL,N1,N2
Cf2py intent(out) tnd,erx,ery,erz,rotx,roty
Cf2py depends(NT) tnd,erx,ery,erz,rotx,roty
...
C       (N .EQ. 1) to (N .EQ. 0) and (KOUNT .EQ. 20) to (KOUNT .EQ. 0) 
...
C       CALL PLTTRA(Y1,Y2,Y3,Y4,Y5,Y6,Y7,Y8,Y9,Y10,DT,T,
C    +              N,NT,FLAG1,FLAG2)
